...
public class Game {
	...
	public static void main(String[] args){
		...
		do{
			...
			try{
				...
				if(playerCount <2){
					gui.displayMessage("This game requires at least 2 players!");
				}
			}catch(NumberFormatException e){
				gui.displayMessage("That's not a number!");
			}
		...
		gui.displayMessage( "Great! " + playerNameList + "let's get started!");
		...
		if(soloWinner)
			gui.displayMessage("Congratulations, " + winner + "! You've won the game with " + mostSets + " sets!");
		else
			gui.displayMessage("There is a tie for first place! " + winner + " both finished the game with " + mostSets + " sets!");
		System.exit(0);
	}
	...
	public static void playRound(Player[] players){
		...
		for(Player currentPlayer : players){
			gui.repaint();
			gui.displayMessage("Ok, " + currentPlayer.getName() + ", it's your turn! Click OK when you are ready!");
			...
			do{
				...
				do{
					...
					if(!currentPlayer.getHand().containsCardValue(guess))
						gui.displayMessage( "You cannot guess a card you do not have.");
				}
				...
				do{
					...
					if(personAsked.equalsIgnoreCase(currentPlayer.getName()))
						gui.displayMessage("You cannot ask yourself for a card");
					opponent = getPlayerByName(personAsked);
					if(opponent == null)
						gui.displayMessage("No player found named " + personAsked +".");
				}
				...
				if(opponent.getHand().containsCardValue(guess)){
					...
					if(cardFromOpponent != null){
						...
						cardMatched = true;
						gui.displayMessage( "You made a match, " + currentPlayer.getName() + ", so your turn continues!");
					}
				}
				else{
					gui.displayMessage( personAsked + " says, \"GeauxFish!\"");
					...
					if(cardFromDeck.getFaceValue().equalsIgnoreCase(guess)){
						cardMatched = true;
						gui.displayMessage( "Lucky! You drew a " + guess + " from the deck! Your turn continues!");
			...
			if(currentPlayer.getHand().size() > 0)
				gui.displayMessage("Your turn is over. Please hit enter to move to the next turn.");
			else{
				gui.displayMessage("Your turn is over. You are out of cards!");
				return;
			}		
		}
	}
